...
| `render_page()` | Rasterize one PDF page to a caller-owned PNG (used by [hover.nvim](https://github.com/StefanBartl/hover.nvim) to preview a PDF page in a float, and by [images.nvim](https://github.com/StefanBartl/images.nvim) to preview one in a picker's preview window) | [Commands](docs/commands.md) |
| `create()` / `:PdfPort create` | Create a PDF from an image, Markdown, text, HTML, or Office file, via 9 creation producers (img2pdf, magick, pandoc, weasyprint, chromium, soffice, qpdf, pdftk, ghostscript). Also how [hover.nvim](https://github.com/StefanBartl/hover.nvim) previews a `.docx`: convert, then rasterize page 1 | [Commands](docs/commands.md) |
...
- **Page rasterization** — `render_page()` renders a single PDF page to a caller-owned PNG file, the primitive other plugins build on to show a PDF page as an image — hover.nvim's PDF hover and images.nvim's PDF preview in a picker's preview window are the two in this ecosystem
