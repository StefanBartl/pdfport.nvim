...
function M.extract(path, opts)
  ...
  local function finish_error(msg)
    local result = {
      status = "error", text = nil, format = "plain", backend = "tesseract",
      pages_processed = page_idx - 1, error = msg,
    }
  ...
  local function process_next()
    if page_idx > #pages then
      local result = {
        ...
        format = "plain", backend = "tesseract",
    ...
    local argv_raster = {
      "pdftoppm", "-png", "-r", tostring(dpi),
      "-f", tostring(page), "-l", tostring(page), "-singlefile", path, tmp,
    }
    ...
    spawn_capture(argv_raster, { timeout_ms = timeout_ms }, function(raster_result)
      ...
      if not raster_result.ok then
        finish_error(string.format("tesseract: pdftoppm failed on page %d: %s", page, raster_result.stderr))
        return
      end
      ...
      spawn_capture({ "tesseract", png, "stdout" }, { timeout_ms = timeout_ms }, function(ocr_result)
        vim.fn.delete(png)
        ...
        if not ocr_result.ok then
          finish_error(string.format("tesseract: OCR failed on page %d: %s", page, ocr_result.stderr))
          return
        end
